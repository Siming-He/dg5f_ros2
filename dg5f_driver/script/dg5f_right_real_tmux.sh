...
IO="${IO:-false}"
SESSION="${DG5F_TMUX_SESSION:-dg5f_right_real}"
...
launch_cmd=$(printf 'ros2 launch dg5f_driver dg5f_right_driver.launch.py delto_ip:=%q delto_port:=%q fingertip_sensor:=%q ft_broadcaster:=%q io:=%q' "$DELTO_IP" "$DELTO_PORT" "$FINGERTIP_SENSOR" "$FT_BROADCASTER" "$IO")
...
tmux select-pane -t "$SESSION:main.0" -T "real driver"
